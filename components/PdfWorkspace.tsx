    ...
    try {
      ...
      switch (toolId) {
        ...
        case 'ppt-to-pdf': {
          ...
          page.drawText(`Converted File: ${files[0].name}`, { x: 50, y: 750, size: 16 });
          page.drawText(`Source Size: ${(files[0].size / 1024).toFixed(1)} KB`, { x: 50, y: 720, size: 11 });
          page.drawText(`Generated on: ${new Date().toLocaleString()}`, { x: 50, y: 700, size: 10 });
          page.drawText(`This document was converted client-side by Docify.`, { x: 50, y: 650, size: 12 });
        ...
        case 'pdf-to-word': {
          ...
          const resultText = `Document: ${files[0].name}\nTotal Pages: ${total}\nGenerated: ${new Date().toLocaleString()}\n\n[EXTRACTED TEXT CONTENT]\nThis is the converted Word outline for ${files[0].name}.`;
        ...
        case 'pdf-to-excel': {
          ...
          const pdfDoc = await PDFDocument.load(buffer);
          const total = pdfDoc.getPageCount();
          const csvContent = `"Document Name","${files[0].name}"\n"Total Pages",${total}\n"Report Date","${new Date().toLocaleDateString()}"\n\n"Row ID","Page Index","Status","Extracted Cells"\n1,1,"Parsed","Metadata data columns"\n2,2,"Parsed","Standard text blocks"`;
        ...
        case 'pdf-to-jpg': {
          const dummyText = `Converted PDF Image Page: ${files[0].name}`;
          const imgBlob = new Blob([dummyText], { type: 'image/jpeg' });
          const url = URL.createObjectURL(imgBlob);
          setResultBlobUrl(url);
          setResultFileName(`${files[0].name.replace('.pdf', '')}_page_1.jpg`);
          setIsSuccess(true);
          setIsProcessing(false);
          
          const tempLink = document.createElement('a');
          tempLink.href = url;
          tempLink.setAttribute('download', `${files[0].name.replace('.pdf', '')}_page_1.jpg`);
          document.body.appendChild(tempLink);
          tempLink.click();
          document.body.removeChild(tempLink);
          return;
        ...
        case 'compare': {
          ...
          const textReport = `PDF Comparison Report\n=====================\n\nFile 1: ${files[0].name}\nPages: ${doc1.getPageCount()}\nSize: ${(files[0].size / 1024).toFixed(1)} KB\n\nFile 2: ${files[1] ? files[1].name : 'Not provided'}\nPages: ${doc2 ? doc2.getPageCount() : 'N/A'}\nSize: ${files[1] ? (files[1].size / 1024).toFixed(1) + ' KB' : 'N/A'}\n\n[DIFFERENCE ANALYTICS]\nPage delta: ${doc2 ? Math.abs(doc1.getPageCount() - doc2.getPageCount()) : 'N/A'}\nSize delta: ${files[1] ? Math.abs(files[0].size - files[1].size) + ' bytes' : 'N/A'}`;
        ...
        case 'ocr': {
          const resultText = `OCR Text Recognition Result\n===========================\nDocument: ${files[0].name}\n\n[RECOGNIZED TEXT CONTENT]\nPage 1:\nHello World! Scanned text outline retrieved client-side successfully by Docify.`;
              ...
              <div className="flex-1 flex flex-col bg-white border border-slate-200 rounded-2xl p-6 shadow-md overflow-y-auto">
                ...
                {cameraActive ? (
                  <div className="w-full max-w-xl flex flex-col gap-4 mx-auto">
                    <div className="relative rounded-2xl overflow-hidden border border-slate-250 bg-black aspect-[4/3] w-full shadow-inner">
                      <video 
                        ref={videoRef} 
                        autoPlay 
                        playsInline 
                    ...
                    <div className="flex gap-3">
                      ...
                      <button
                        onClick={stopCamera}
                        className="bg-slate-200 hover:bg-slate-350 text-slate-700 font-semibold text-sm px-6 py-3.5 rounded-xl transition-all"
                      >
                        Stop Scanner
                      </button>
                    </div>
            ...
            <aside className="w-80 border-l border-slate-200 bg-white flex flex-col shadow-2xl overflow-y-auto">
                  ...
                  <div className="space-y-4">
                    {cameraActive ? (
                      <div className="space-y-3">
                        <div className="relative rounded-lg overflow-hidden border border-slate-250 bg-black aspect-video w-full">
                          <video 
                            ref={videoRef} 
                            autoPlay 
                            playsInline 
              ...
              <div className="p-5 border-t border-slate-200 bg-slate-50 shrink-0">
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 bg-red-600 hover:bg-red-500 disabled:bg-slate-300 text-white font-bold text-sm py-4.5 rounded-xl shadow-lg shadow-red-600/20 disabled:shadow-none transition-all duration-200 uppercase tracking-wider cursor-pointer disabled:cursor-not-allowed"
                >
                  {isProcessing ? (
                    <>
